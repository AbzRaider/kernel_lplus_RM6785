...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		scnprintf(buf, 50, "aal_dbg_en = 0x%x\n", aal_dbg_en);
		...
		scnprintf(buf, 50, "color_dbg_en = 0x%x\n", debug_level);
		...
		scnprintf(buf, 50, "corr_dbg_en = 0x%x\n", corr_dbg_en);
