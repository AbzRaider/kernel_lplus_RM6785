...
/* ZhongWenjie@BSP.TP.FUNCTION, 2018/11/19, add to enable tp headset mode when plug in */
...
static void dump_register(void)
{
	...
	for (i = ACCDET_RSV; i <= ACCDET_EINT1_CUR_DEB; i += 2)
		pr_debug("ACCDET_ADDR:(0x%x)=0x%x\n", i, pmic_read(i));
...
#endif
	pr_debug("%s done IRQ-STS[0x%x]=0x%x,STATE_SWCTRL[0x%x]=0x%x\n",
...
static inline void headset_plug_out(void)
{
	...
	pr_debug("accdet %s, set cable_type = NO_DEVICE\n", __func__);
#if PMIC_ACCDET_DEBUG
	dump_register();
#endif
}
...
static void dis_micbias_work_callback(struct work_struct *work)
{
	if (cable_type == HEADSET_NO_MIC) {
		...
		disable_accdet();
	pr_debug("%s more than 6s,MICBIAS:Disabled c_type:0x%x\n",
		__func__, cable_type);
	}
}
#endif /* end of #if PMIC_ACCDET_KERNEL */
...
#endif /* CONFIG_HSKEY_BLOCK */
			#ifdef VENDOR_EDIT
			/* Jingjing.Jiang@PSW.MM.AudioDriver.Machine.2649660, 2020/01/14,
			 * add for dump log out of workqueue */
			if (door_open == 1) {
					pr_debug("%s: enter dump\n", __func__);
					BUG_ON(1);
			}
			#endif /* VENDOR_EDIT */
			pr_debug("[accdet_eint_func]delayed work 500ms scheduled when plugging in\n");
...
#endif /* CONFIG_HSKEY_BLOCK */
			pr_debug("[accdet_eint_func]delayed work 0ms scheduled when plugging out\n");
...
static u32 moisture_detect(void)
{
	...
	pr_debug("%s accdet Moisture Read Auxadc=%d\n", __func__, moisture_vol);
	...
	if ((irq_status & ACCDET_IRQ_B0) && (eintID == 0)) {
		...
		if (water_r != 0) {
			...
			if (moisture_vol > moisture_vm) {
				...
				clear_accdet_eint(eintID);
				clear_accdet_eint_check(eintID);
				pr_debug("%s Moisture plug in detectecd!\n",
					__func__);
				return;
			}
...
static int accdet_get_dts_data(void)
{
	...
	pr_debug("accdet mic_vol=%d, plugout_deb=%d mic_mode=%d eint_pol=%d\n",
