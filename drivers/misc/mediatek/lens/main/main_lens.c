...
/* WenJun@Cam.Drv, 20200103, add for 19131 AF*/
...
/*GaoWang@Cam.Drv add for 20682 AF BringUp 20200930*/
...
#endif /* CONFIG_MACH_MT6853 || CONFIG_MACH_MT6873 */
...
void AFRegulatorCtrl(int Stage)
{
    ...
    if (is_project(20610) || is_project(20611) || is_project(20613) || is_project(20680) ) {
            Other_AFRegulatorCtrl(Stage);
            return;
    }
	...
	if (Stage == 0) {
		...
		if (g_regVCAMAFEn == 0) {
			if (is_project(20601) || is_project(20602) || is_project(20660)) {
                            pmic_ldo_get_type();
			    Status = pmic_ldo_set_voltage_mv(6, 1800);
		...
		if (g_regVCAMAFEn == 1) {
			if (is_project(20601) || is_project(20602) || is_project(20660)) {
			        ...
			        Status = pmic_ldo_set_disable(6);
						...
						|| is_project(20633) || is_project(20634) || is_project(20635)) {
			//Status = fan53870_cam_ldo_disable(5);
			} else {
                                pmic_ldo_get_type();
