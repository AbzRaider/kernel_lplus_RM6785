...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = (return_sensor_id() + 1);
            ...
            if ((*sensor_id == imgsensor_info.sensor_id)
                ...
                printk("OV02B1B get_imgsensor_id success: 0x%x\n", *sensor_id);
                #ifdef OPLUS_FEATURE_CAMERA_COMMON
                /*Degao.Lan@Camera.DRV add for register device info 20191108*/
                imgsensor_info.module_id = IMGSENSOR_MODULE_ID_SHINE;
                if (deviceInfo_register_value == 0x00) {
                    Oplusimgsensor_Registdeviceinfo("Cam_r3", DEVICE_VERSION_OV02B1B, imgsensor_info.module_id);
    ...
    if ((*sensor_id != imgsensor_info.sensor_id)
        || imgsensor_info.module_id == IMGSENSOR_MODULE_ID_HOLITECH) {
        printk("OV02B1B get_imgsensor_id failed: 0x%x, %d\n", *sensor_id, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = (return_sensor_id() +1);
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("OV02B1B open success: 0x%x\n", sensor_id);
                break;
            }
    ...
    if (imgsensor_info.sensor_id != sensor_id) {
        printk("OV02B1B open failed: 0x%x\n", sensor_id);
...
{
	pr_debug("[ov02b1b] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug("[ov02b1b] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug("[ov02b1b] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug("[ov02b1b] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug("[ov02b1b] slim_video mode start\n");
