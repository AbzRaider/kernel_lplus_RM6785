...
static struct imgsensor_info_struct imgsensor_info = {
	 ...
	 * use same framelength, linelength with Capture mode for
	 * shutter calculate
	 */
	.cap1 = {
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] = {
	{ 4032, 3016, 0, 0, 4032, 3016, 2016, 1508,
	  0, 0, 2016, 1508, 0, 0, 2016, 1508},/* Preview */
	{ 4032, 3016, 0, 0, 4032, 3016, 4032, 3016,
	  0, 0, 4032, 3016, 0, 0, 4032, 3016}, /* capture */
	{ 4032, 3016, 0, 0, 4032, 3016, 4032, 3016,
	  0, 0, 4032, 3016, 0, 0, 4032, 3016}, /* normal video*/
	{ 4032, 3016, 720, 772, 2592, 1472, 1296,
	  736, 0, 0, 1296, 736, 0, 0, 1296, 736}, /* high speed video */
	{ 4032, 3016, 720, 772, 2592, 1472, 1296,
	  736, 0, 0, 1296, 736, 0, 0, 1296, 736}, /* slim video */
	{ 4032, 3016, 0, 0, 4032, 3016, 4032, 3016,
	  0, 0, 4032, 3016, 0, 0, 4032, 3016}, /* custom1 */
	{ 4032, 3016, 0, 0, 4032, 3016, 4032, 3016,
	  0, 0, 4032, 3016, 0, 0, 4032, 3016}, /* custom2 */
	{ 4032, 3016, 0,  380, 4032, 2256, 4032,
	  2256, 0, 0, 4032, 2256, 0, 0, 4032, 2256}, /* custom3 */
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4SubBlkH = 16,
	.i4PosL = {
	  {28, 31}, {80, 31}, {44, 35}, {64, 35}, {32, 51}, {76, 51}, {48, 55},
	  {60, 55}, {48, 63}, {60, 63}, {32, 67}, {76, 67}, {44, 83}, {64, 83},
	  {28, 87}, {80, 87} },
	.i4PosR = {
	  {28, 35}, {80, 35}, {44, 39}, {64, 39}, {32, 47}, {76, 47}, {48, 51},
...
/*  */
/* .i4PosL =
 * {{31,28},{83,28},{47,32},{67,32},{35,40},{79,40},{51,44},{63,44},{51,60},
 * {63,60},{35,64},{79,64},{47,72},{67,72},{31,76},{83,76}},
 */
/*  */
	...
	kal_uint32 addr, u8 *reg_buf, kal_uint32 size)
{
	...
	for (i = 0; i < size; i += MAX_READ_WRITE_SIZE) {
		...
		if ((i + MAX_READ_WRITE_SIZE) > size) {
			...
			ret = iBurstWriteReg((u8 *)&buf,
				(size - i + 2), imgsensor.i2c_write_id);
		} else {
			memcpy(buf.data, (reg_buf + i), MAX_READ_WRITE_SIZE);
			ret = iBurstWriteReg((u8 *)&buf,
				(MAX_READ_WRITE_SIZE + 2),
...
{
	memcpy((void *)data,
		(void *)(bayer_primax_otp_buf + PDAF_CAL_DATA_OFFSET),
		PDAF_CAL_DATA_SIZE);
}
...
static void load_imx386_spc_data(void)
{
	...
	memcpy((void *)&puSendCmd[tosend],
		(void *)(bayer_primax_otp_buf + SPC1_START_ADDR),
		SPC_DATA_SIZE);
	...
	for (i = 0; i < SPC_DATA_SIZE; i++)
		write_cmos_sensor(0x7D4C+i,
			bayer_primax_otp_buf[SPC1_START_ADDR + i]);
#endif
#if 0
	for (i = 0; i < SPC_DATA_SIZE; i++)
		write_cmos_sensor(0x7D80+i,
			bayer_primax_otp_buf[SPC2_START_ADDR + i]);
#endif
}
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_LENS_DRIVER_ID:
		 ...
		 * LENS_DRIVER_ID_DO_NOT_CARE if EEPROM does not exist
		 * in camera module.
		 */
