...
** Description: Source file for virtual sensor linux driver.
** Version: 1.0
** Date : 2020/03/31
...
void oppo_init_sensor_state(struct SensorState *mSensorState)
{
    ...
    mSensorState[SENSOR_TYPE_OPPO_ACTIVITY_RECOGNITION].sensorType =
        SENSOR_TYPE_OPPO_ACTIVITY_RECOGNITION;
    mSensorState[SENSOR_TYPE_OPPO_ACTIVITY_RECOGNITION].rate = SENSOR_RATE_ONCHANGE;
    ...
    strlcpy(p->vendor, "oppo", sizeof(p->vendor));
    ...
    #ifdef CONFIG_OPLUS_FEATURE_ACTIVITY_RECOGNITION
    //pengqingqing@ANDROID.AITECH,2020/09/07, add for oppo_activity_recognition
    p = &mSensorState[SENSOR_TYPE_OPPO_ACTIVITY_RECOGNITION];
    p->gain = 1;
...
static int handle_to_index(int handle)
{
    ...
    switch (handle) {
    /*xiebaixue@PSW.BSP.Sensor, 2020/04/16, Add for oppo algo*/
    ...
    #ifdef CONFIG_OPLUS_FEATURE_ACTIVITY_RECOGNITION
    //pengqingqing@ANDROID.AITECH,2020/07/01, add for oppo_activity_recognition
    case ID_OPPO_ACTIVITY_RECOGNITION:
        index = oppo_activity_recognition;
        break;
    #endif //CONFIG_OPLUS_FEATURE_ACTIVITY_RECOGNITION
    ...
    size_t count, loff_t *ppos)
{
    ...
    read_cnt = oppo_sensor_event_read(virtual_sensor_context_obj->mdev.minor,
...
{
    return oppo_sensor_event_poll(virtual_sensor_context_obj->mdev.minor, file, wait);
}
...
static int virtual_sensor_misc_init(struct virtual_sensor_context *cxt)
{
    ...
    err = oppo_sensor_attr_register(&cxt->mdev);
    if (err) {
...
int virtual_sensor_data_report(struct oppo_sensor_event event)
{
    ...
    err = oppo_sensor_input_event(virtual_sensor_context_obj->mdev.minor, &event);
...
static int virtual_sensor_remove(void)
{
    ...
    sysfs_remove_group(&virtual_sensor_context_obj->mdev.this_device->kobj,
        &virtual_sensor_attribute_group);
    err = oppo_sensor_attr_deregister(&virtual_sensor_context_obj->mdev);
    if (err) {
