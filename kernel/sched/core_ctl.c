...
extern int sched_get_nr_running_avg2(int *avg, int *iowait_avg,
	unsigned int *max_nr, unsigned int *big_max_nr);
static void update_running_avg(void)
{
	int avg, iowait_avg;
	int l_big_avg, big_avg;
	int max_nr, big_max_nr;
	struct cluster_data *cluster;
	unsigned int index = 0;
	unsigned long flags;

	avg = iowait_avg = l_big_avg = big_avg = max_nr = big_max_nr = 0;
	sched_get_nr_running_avg2(&avg, &iowait_avg, &max_nr, &big_max_nr);
	sched_big_task_nr(&l_big_avg, &big_avg);

	trace_core_ctl_update_running_avg(avg, iowait_avg, max_nr, big_max_nr,
		l_big_avg, big_avg);
	spin_lock_irqsave(&state_lock, flags);
